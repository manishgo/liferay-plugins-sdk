...
/**
 * The Product Registration Portlet allows customers to register their
 * products electronically without having to fill out the reply card
 * and send it back.
 *
 * @author Rich Sezov
 *
...
public class ProductRegistrationPortlet extends MVCPortlet {
    ...
    /**
     * Takes a submitted registration and adds it to the database if it
     * passes validation.
     * @param request
     * @param response
     */
    public void registerProduct (ActionRequest request, ActionResponse response) throws Exception {
        ...
        if (userValid && regValid) {
            ...
            //TODO: Logic needs to change here for multiple registrations and to check for logged-in users
            PRUser user = null;
            ...
            if (liferayUser.isDefaultUser()) {
                ...
                user = PRUserLocalServiceUtil.getRegUser(themeDisplay.getScopeGroupId(), userId);
            ...
            PRRegistration reg = PRRegistrationLocalServiceUtil.addRegistration(registration);
